...
class Options():
    ...
    def init(self, parser):
        ...
        parser.add_argument('--pretrain_weights', type=str, default='path to pretrain_weights.pth',
                            help='path of pretrained_weights')
        parser.add_argument('--optimizer', type=str, default='muon', help='optimizer for training')
        parser.add_argument('--criterion', type=str, default='L1', help='criterion for training')
        ...
        parser.add_argument('--arch', type=str, default='Lith4SR', help='architecture')
        parser.add_argument('--mode', type=str, default='shadow', help='image restoration mode')
        parser.add_argument('--lam_align', type=float, default=0.1, help='align_loss rate')

        # args for saving
        parser.add_argument('--save_dir', type=str, default='./log', help='save dir')
        parser.add_argument('--save_images', action='store_true', default=False)
        parser.add_argument('--env', type=str, default='env_name', help='env')
        parser.add_argument('--checkpoint', type=int, default=300, help='checkpoint')
        ...
        parser.add_argument('--train_dir', type=str, default='Path to the training data directory', help='dir of train data')
        parser.add_argument('--val_dir', type=str, default='Path to the validation data directory, which should contain test_A, test_B, and test_C folders', help='dir of val data')
        parser.add_argument('--warmup', action='store_true', default=True, help='warmup')
        parser.add_argument('--warmup_epochs', type=int, default=30, help='epochs for warmup')

        parser.add_argument('--finetune', action='store_true', default=False,
                    help='Load weights only; reset epoch/best/optimizer/scheduler for a new dataset')
        parser.add_argument('--reset_best', action='store_true', default=False,
                    help='Force best_psnr=0 even when --resume is set')
        parser.add_argument('--epochs_to_run', type=int, default=0,
                    help='If >0, train for this many epochs from start_epoch (overrides absolute --nepoch end)')
