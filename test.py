...
parser.add_argument('--input_dir', default='path to input dir',
    type=str, help='Directory of validation images')
parser.add_argument('--result_dir', default='path to result dir',
    type=str, help='Directory for results')
parser.add_argument('--weights', default='path to pretrain weight',
    type=str, help='Path to weights')
parser.add_argument('--gpus', default='0', type=str, help='CUDA_VISIBLE_DEVICES')
parser.add_argument('--arch', default='Lith4SR', type=str, help='arch')
...
parser.add_argument('--no_mask', action='store_true', help='Set if dataset has no mask (test_B missing)')
...
has_mask = not args.no_mask

test_dataset = get_validation_data(args.input_dir, return_mask=has_mask)
...
with torch.no_grad():
    ...
    for ii, data_test in enumerate(tqdm(test_loader), 0):
        if has_mask:
            ...
            hsl_noisy = data_test[2].cuda()
            mask = data_test[3].cuda()
        ...
        hsl_noisy = F.pad(hsl_noisy, (0, padw, 0, padh), 'reflect')
        if has_mask:
            mask = F.pad(mask, (0, padw, 0, padh), 'reflect')

        if args.tile is None:
            rgb_restored = model_restoration(rgb_noisy, hsl_noisy)
        else:
            ...
            for h_idx in h_idx_list:
                for w_idx in w_idx_list:
                    ...
                    hsl_patch = hsl_noisy[..., h_idx:h_idx + tile, w_idx:w_idx + tile]
                    out_patch = model_restoration(in_patch, hsl_patch)
...
if args.cal_metrics:
    psnr_val_rgb = sum(psnr_val_rgb)/len(test_dataset) if len(psnr_val_rgb)>0 else 0
    ssim_val_rgb = sum(ssim_val_rgb)/len(test_dataset) if len(ssim_val_rgb)>0 else 0
    rmse_val_rgb = sum(rmse_val_rgb) / len(test_dataset) if len(rmse_val_rgb)>0 else 0
